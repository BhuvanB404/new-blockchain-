...
async function main() {
    try {
        ...
        const userIdentity = await wallet.get('LabOverseer01');
        if (userIdentity) {
            console.log('An identity for the user "LabOverseer01" already exists in the wallet');
            return;
        }
        ...
        const secret = await ca.register({
            ...
            enrollmentID: 'LabOverseer01',
            role: 'client',
            attrs: [
                { name: 'role', value: 'labOverseer', ecert: true },
                { name: 'uuid', value: 'LabOverseer01', ecert: true }
            ],
        }, adminUser);
        ...
        await wallet.put('LabOverseer01', x509Identity);
        console.log('Successfully registered and enrolled lab overseer user "LabOverseer01" and imported it into the wallet');
    } catch (error) {
        console.error(`Failed to register user "LabOverseer01": ${error}`);
        process.exit(1);
    }
}
